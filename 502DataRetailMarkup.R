...
str(from_topshelf$Category)




summary(x$stats[3,])[3] # This pulls out each of the IQRs for all the of the product's medians
y<-summary(x$stats[3,])
y[3]
...
tapply(from_topshelf$Wholesale.Sales, from_topshelf$Category, FUN=sum)

from_topshelf %>% group_by(Category) %>% summarise(result=min(Wholesale.Sales))
